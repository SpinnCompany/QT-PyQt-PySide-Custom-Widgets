## QCustomMainWindow - the legacy app window (kept for `from Custom_Widgets
## import *` example compatibility). Lived in Custom_Widgets/__init__.py; it
## now lives here so the package root can import without pulling in the whole
## Qt + theme stack (design linter, stubgen and docs tools only need the root).
## v3 rename: this class used to SHADOW Qt's QMainWindow when star-importing
## Custom_Widgets - subclass QCustomMainWindow explicitly instead.
import re

from qtpy.QtCore import QCoreApplication, Qt, QSettings, QPoint, QSize, QEvent
from qtpy.QtGui import QCursor, QPaintEvent, QColor, QMouseEvent, QPainter
from qtpy.QtWidgets import QMainWindow, QWidget, QStyle, QStyleOption, \
...
class QCustomMainWindow(QMainWindow):
    ...
    drag/resize, side drawers and live QSS."""
    ...
    def getHamburgerMenu(self, name: str) -> QWidget:
        from Custom_Widgets.QCustomHamburgerMenu import QCustomHamburgerMenu
